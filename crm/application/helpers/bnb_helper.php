    ...
    if($all)
    {
        foreach ($all as $type => $messages)
        {
            foreach ($messages as $message)
            {
                ...
                $msg='<div class="alert  alert-dismissable '.$type.'"><a href="" class="close" data-dismiss="alert" aria-label="close">×</a>'.$icon.' '.$message.'</div>';
            }
        }
        return $msg;
    }
...
function image_upload($folder,$maxheight,$maxwidth,$resizeheight='',$resizewidth='')
{
    if($resizeheight=='')
    {
        $resizeheight=50;
    ...
    if(!$CI->upload->do_upload('files'))
    {
        ...
        $error = array('status'=>false,'msg' => $CI->upload->display_errors());
        return $error;
    }
    else
    {
        $data = array('upload_data' => $CI->upload->data());
        $uploadedimagename= $data['upload_data']['file_name'];

        copy("uploads/".$folder."/".$uploadedimagename,"uploads/".$folder."/resize/".$uploadedimagename);


        $config['image_library'] = 'gd2';
        $config['source_image'] = './uploads/'.$folder.'/resize/'.$data['upload_data']['file_name'];
        ...
        $CI->image_lib->resize();
        $msg = array('status'=>true,'msg' => $uploadedimagename);
        return $msg;
    }
...
{
    $CI =& get_instance();
    $CI->db->where('bankId',$id);
    $query = $CI->db->get('banks');
    $result =  $query->result();
    return $result;
}
function get_all_bank($id="")
{
    $CI =& get_instance();
    $query = $CI->db->get('banks');
    $result =  $query->result();
...
{
    $CI =& get_instance();
    $CI->db->where('SiptmUserID',$id);
    $query = $CI->db->get('nominee');
    $result =  $query->result();
    return $result;
}
function getiplist($id="")
{
    $CI =& get_instance();
    $CI->db->where('login_user_id',$id);
    $query = $CI->db->get('login_history');
    $result =  $query->result();
    return $result;
}
...
function get_active_holding($id='')
{
    $CI =& get_instance();
    $CI->db->where('client_id',$id);
